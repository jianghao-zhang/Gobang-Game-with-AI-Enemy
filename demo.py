...
class Game():
	def __init__(self, caption, play_mode, AI_first):
		...
		self.mode = play_mode
		self.buttons = []
		self.buttons.append(StartButton(self.screen, 'Start', MAP_WIDTH + 30, 250))
		self.buttons.append(GiveupButton(self.screen, 'Giveup', MAP_WIDTH + 30, BUTTON_HEIGHT + 300))
		self.is_play = False
	...
	def play(self):
		...
		color_board = (177, 177, 177)
		pygame.draw.rect(self.screen, color_board, pygame.Rect(0, 0, MAP_WIDTH, SCREEN_HEIGHT))
	...
	def showAIThink(self):
		self.showFont('AI is thinking...', MAP_WIDTH + 25, SCREEN_HEIGHT//2+70, 30)
...
game = Game("Gobang with AI", GAME_PLAY_MODE, AI_RUN_FIRST)
while True:
	game.play()
